...
LR=LR(b);
x=t1:t2;
plot(x,RES(t1:t2),x,STO(t1:t2),x,EXC1(t1:t2),x,EXC2(t1:t2),x,SP(t1:t2),x,SC(t1:t2),[t1,t2],[LH,LH],':',[t1,t2],[LB,LB],':',[t1,t2],[LR,LR],'-.','LineWidth',1.5) 
 legend('Network Extr. (RES)','Storage (STO)','Surplus 1 (EXC1)','Surplus 2 (EXC2)','Production (SP)','Consumption (SC)','Upper limit (LH)','Lower limit (LB)','Network limit (LR)','location','eastoutside')
 xlabel('Time [hours]')
 ylabel('Power [kW]')